...
`npm run build` (`tsc -b && vite build`) is the primary correctness check; `npm run lint` runs oxlint. There is no test suite. Verification is `npm run build` + `npm run lint` +
