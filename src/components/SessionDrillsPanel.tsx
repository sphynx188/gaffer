  ...
  return (
    <div className="mt-3 border-t border-slate-200 pt-3">
      {attachedDrills.length === 0 ? (
        <p className="text-sm text-slate-400">No drills attached yet.</p>
      ) : (
      ...
      <form onSubmit={handleAttach} className="mt-3 flex flex-wrap items-end gap-2 border-t border-slate-100 pt-3">
        <div className="min-w-40 flex-1">
          <label className="block text-xs font-medium text-slate-500">Drill</label>
          <select
      ...
      {attachError && <p className="mt-1 text-sm text-red-600">{attachError}</p>}
    </div>
  )
}
...
function SessionDrillRow({
  ...
  return (
    <li className="rounded-md border border-slate-200 px-3 py-2">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-sm font-medium text-slate-900">
            <span className="mr-1.5 text-slate-400">{position}.</span>
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          <button
            ...
            disabled={isFirst}
            aria-label="Move up"
            className="rounded-md border border-slate-300 px-1.5 py-0.5 text-xs text-slate-600 disabled:opacity-30"
          >
            ↑
          </button>
          <button
      ...
      {rowError && <p className="mt-1 text-sm text-red-600">{rowError}</p>}
    </li>
  )
}
